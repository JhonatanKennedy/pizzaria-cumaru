...
- **`packages/` has not been created.** The API contract is still hand-mirrored: `apps/web/src/api/*.api.ts` re-declares the wire shapes in zod and const unions (`ORDER_TYPES`) while `apps/api/src/**/domain/enums/*` declares the same values as TS enums (`EOrderType`, `EOrderStatus`, `EUserRole`). Hoisting them into a shared package is the next planned step — the runbook's Phase 6 in [docs/monorepo-migration.md](docs/monorepo-migration.md), still unstarted.
- **The product is described at two altitudes.** `features/` holds the Gherkin; `openspec/specs/` holds the 22 capability specs — 8 UI-level (`waiter-table-orders`, `kitchen-panel`, `manager-menu-stock`, `manager-table-management`, `delivery-orders`, `close-table-order`, `daily-sales`, `daily-earnings-report`) and 14 domain-level (`orders/` in seven sub-specs, plus `catalog/`, `kitchen/`, `tables/`, and `users/` and `authorization/` in two each). The union made the redundancy visible rather than resolving it; a change that restates an existing capability gets retargeted onto it rather than minting a duplicate — `manager-delivery-orders` did, because it restated `delivery-orders` + `orders/delivery-status` + `orders/order-creation`.
- **`openspec/changes/` is empty; the archive holds 33 changes.** The 13 left over from before the merge were archived on 2026-09-10 in a deliberate pass, not a bulk `git mv` — every delta was checked against the code first, and six needed work. Four had been written against behavior a later change reversed — three against the cancellation `reason` (`drop_cancellation_reason`), one against the price-only and per-ingredient item routes that collapsed into a single `PATCH /items/:itemId`. `manager-delivery-orders` was retargeted onto the existing `delivery-orders` capability. `order-and-menu-polish`'s `close-table-order` delta was rewritten to carry its two shipped requirements as `MODIFIED`, so it converges as a no-op over the capability `manager-close-order` creates instead of colliding with it. Because a delta cannot rewrite a capability's Purpose paragraph, `waiter-table-orders`, `kitchen-panel` and `delivery-orders` had theirs edited directly during the sync — keep that in mind before writing a change whose Purpose turns out to be wrong.
