...
Closing an open table order with a payment method is the manager's verb in the SPA: on the order detail screen shared with waiters, the manager may close the order (Dinheiro/Cartão/Pix), turning it "Fechada" and freeing its table; waiters never see the action. Traces to `features/07_manager_profile.feature` and `features/05_waiter_profile.feature`.
