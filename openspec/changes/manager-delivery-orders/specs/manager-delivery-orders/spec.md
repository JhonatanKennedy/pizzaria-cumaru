...
The manager's delivery orders: an order received by WhatsApp — customer name, phone and address, no table — gets items while it is open and advances step by step (`Open → Preparing → Out for delivery → Delivered`), with the delivered time recorded. Manager-only in the SPA by product decision; the flow traces to `features/04_delivery_order.feature`.
