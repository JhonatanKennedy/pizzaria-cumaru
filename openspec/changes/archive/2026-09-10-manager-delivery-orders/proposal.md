...
<!-- none — the SPA delivery capability already exists -->

### Modified Capabilities

- `delivery-orders`: the SPA delivery capability gains the flow it was missing — the manager's list, the create dialog, and a detail screen with one advance action per step until "Delivered", with the delivered time shown. The waiter/order capabilities are unchanged in behavior; this change only moves their contracts to shared modules.
