...
The cancellation `reason` this change introduced was removed afterwards: `order-and-catalog-polish` dropped the `reason` column (`drop_cancellation_reason` migration) and `order-and-menu-polish` took the field out of the SPA dialogs. The `cancelledReason` column named above no longer exists — `Order` carries only `cancelledAt`. The deltas in `specs/` describe the shipped, reason-free behavior.
