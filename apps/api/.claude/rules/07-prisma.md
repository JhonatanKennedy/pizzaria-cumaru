...
`PrismaModule` is `@Global()`, so any provider gets it via constructor injection (see [05-nestjs.md](05-nestjs.md#5-constructor-injection--never-new-a-nest-provider)). Nest's DI is the only thing that may instantiate it — except the e2e suite under `test/`, whose `vitest.config.e2e.ts` builds the real client against the test database. The repository specs do not: they hand the adapter a `vi.fn()`-per-method double and assert the `where` it builds, which is why they run with no database at all.

## 4. Schema mirrors aggregates; enums persist as strings

`schema.prisma` models every aggregate — 9 models: `User`, `DeniedToken`, `Order`, `OrderItem`, `OrderCancellation`, `Item`, `Ingredient`, `ItemIngredient`, `Table` — with a single committed baseline migration under `apps/api/migrations/` (the path `prisma.config.ts` points at; migrations are not under `src/prisma/`). Conventions:
...
`migrations/` exists and is committed, holding **one baseline**, `20260911020000_init`. The scaffold had grown nine (the schema, then eight increments — including a `flavors` `TEXT[] → JSONB` rewrite and a `cancelledReason` column added and dropped three migrations later); they were squashed into that single file, generated with `npx prisma migrate diff --from-empty --to-schema src/prisma/schema.prisma --script`. **A database migrated before the squash keeps its nine rows in `_prisma_migrations` and must be reset** (`npx prisma migrate reset` — it drops and recreates, then re-seeds) — the baseline's name is not in that history, so `migrate dev` reports drift until it is.

From here, extend the schema, then run `npx prisma migrate dev --name <what-changed>`; commit both the migration and the regenerated client like any source file. Never hand-edit a committed migration or `src/prisma/generated/`. `src/prisma/seed.ts` (via `npm run seed`) upserts the three seed users with bcrypt-hashed passwords and seeds the catalog — 12 ingredients, 20 items and 66 item-ingredient links, plus 10 tables. The dataset it writes is `src/prisma/seed-data.ts`, which imports no Prisma and so is checkable by `seed-data.spec.ts` without a database; `seed.ts` itself is only the loops. The item count is higher than `SEED_ITEMS` (10) because `SEED_ITEMS_WITH_SIZES` expands each pizza into one variant per `PIZZA_SIZES` member (`G`/`M`), at the base price. Rows are matched by name (`findFirst`) since catalog names have no DB unique constraint; the seed refuses to run when `NODE_ENV=production` — it is code, not a migration.
