...
- [ ] 6.5 Archive-time — main-spec Purpose paragraphs cannot change via deltas: when syncing/archiving this change, update the `waiter-table-orders` (and any other) main-spec Purpose that still describes cancellations as reason-requiring, so main specs and behavior agree
- [ ] 6.6 Archive-time — when syncing `close-table-order`, keep the two requirements carried verbatim from the active `manager-close-order` change untouched so that change's later archive sync converges as a no-op instead of colliding
